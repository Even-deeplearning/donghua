...
% 这段代码是一个GUI界面的主文件，用于创建一个GUI界面并处理用户的交互事件
...
% Last Modified by GUIDE v2.5 13-Sep-2023 12:39:45
...
guidata(hObject, handles);
InitAxes(handles)
axes(handles.axes1); imshow(handles.Result.Image); title('输入图像');
...
if ~isempty(handles.Result)
    axes(handles.axes1); imshow(handles.Result.Image); title('输入图像');
    axes(handles.axes2); imshow(handles.Result.hist); title('直方图均衡化后的图像');
end
...
if ~isempty(handles.Result)
    axes(handles.axes1); imshow(handles.Result.Image); title('输入图像');
    axes(handles.axes2); imshow(handles.Result.Medfilt); title('中值滤波后的图像');
end
...
if ~isempty(handles.Result)
    axes(handles.axes1); imshow(handles.Result.Image); title('输入图像');
    axes(handles.axes2); imshow(handles.Result.Bw); title('二值化处理后的图像');
end
...
if ~isempty(handles.Result)
    axes(handles.axes1); imshow(handles.Result.Image); title('输入图像');
    axes(handles.axes2); imshow(handles.Result.Enance); title('对比度增强后的图像');
end
...
if ~isempty(handles.Result)
    axes(handles.axes1); imshow(handles.Result.Image); title('输入图像');
    axes(handles.axes2); imshow(handles.Result.Bw); title('二值化处理后的图像');
    axes(handles.axes3); imshow(handles.Result.BwFilter); title('二值化图像滤波后的图像');
end
...
if ~isempty(handles.Result)
    ...
    axes(handles.axes2); imshow(handles.Result.Bw); title('二值化处理后的图像');
    axes(handles.axes3); imshow(handles.Result.BwFilter); title('二值化图像滤波后的图像');
    axes(handles.axes4); imshow(handles.Result.CrackRec); title('识别裂缝');
end
...
if ~isempty(handles.Result)
    axes(handles.axes1); imshow(handles.Result.Image); title('输入图像');
    axes(handles.axes2); imshow(handles.Result.BwFilter); title('二值化图像滤波后的图像');
    axes(handles.axes3); imshow(handles.Result.CrackRec); title('识别裂缝');
    axes(handles.axes4); imshow(handles.Result.CrackJudge); title('判断裂缝');
end
...
if ~isempty(handles.Result)
    ...
    axes(handles.axes4); imshow(handles.Result.CrackJudge); title('裂缝检测图像');
    hold on;
...
if ~isempty(handles.Result)
    axes(handles.axes1); imshow(handles.Result.Image); title('输入图像');
    axes(handles.axes2); imshow(handles.Result.BwFilter); title('二值化图像滤波后的图像');
    axes(handles.axes3); imshow(handles.Result.CrackJudge); title('判断裂缝');
...
if ~isempty(handles.File)
    F = [];
    F{1} = sprintf('输入的裂缝图像');
    F{2} = sprintf('阈值大小 %.2f', handles.Result.BwTh);
    F{3} = sprintf('裂缝面积 %.2f', handles.Result.BwArea);
    F{4} = sprintf('裂缝长度 %.2f', handles.Result.BwLength);
    F{5} = sprintf('裂缝最大宽度 %.2f', handles.Result.BwWidthMax);
    F{6} = sprintf('裂缝最小宽度 %.2f', max(handles.Result.BwWidthMin,0.01));
    F{7} = sprintf('裂缝形状 %s', handles.Result.str);
...
function pushbuttonOpenFile_DeleteFcn(hObject, eventdata, handles)
% hObject    handle to pushbuttonOpenFile (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
